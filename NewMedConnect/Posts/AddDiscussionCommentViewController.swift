    ...
    @IBAction func postTapped(_ sender: Any) {
        ...
        { [self]
            ...
            else
            {
                ...
                db.collection(conditionSelected).document(discussionDocument).collection("comments").document("\(documentsCount)").setData(["commentTitle" : responseTextView.text!, "upvotes": 0, "downvotes": 0, "date": dateFormatter.string(from: date), "user": Auth.auth().currentUser!.uid])
